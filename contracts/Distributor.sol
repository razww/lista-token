...
/**
    ...
    */
contract Distributor is Initializable, AccessControlUpgradeable {
    ...
    TokenAmount[10][65535] weeklyRewardData;
    // token -> reward token index
    mapping(address => uint8) private rewardToken;
    // index -> start week
    RewardToken[10] private rewardTokens;
    // account -> token -> week
    mapping(address => mapping(address => uint16)) accountRewardWeek;
    ...
    function registerNewToken(address _token) external onlyRole(MANAGER) {
        ...
        require(idx < rewardTokens.length, "no more reward tokens");
        rewardTokens[idx] = RewardToken({
            token: _token,
            startWeek: veLista.getCurrentWeek()
        });
        rewardToken[_token] = idx;

        emit RewardRegistered(_token, rewardToken[_token]);
    }
    ...
    function depositNewReward(uint16 _week, TokenAmount[] memory _tokens) external onlyRole(MANAGER) {
        ...
        for (uint8 i = 0; i < _tokens.length; i++) {
            uint8 tokenIdx = rewardToken[_tokens[i].token];
            ...
            weeklyRewardData[_week][tokenIdx] = TokenAmount({
    ...
    function getClaimable(address _account) public view returns (TokenAmount[] memory) {
        ...
        for (uint8 i = 1; i < rewardTokens.length; ++i) {
            ...
            uint16 accountWeek = accountRewardWeek[_account][token];
            if (accountWeek == 0) {
                accountWeek = rewardTokens[i].startWeek;
            }
            for (uint16 j = accountWeek; j < currentWeek; ++j) {
                TokenAmount memory reward = weeklyRewardData[j][i];
                if (reward.amount == 0) {
                    continue;
                }
    ...
    function getTokenClaimable(address _account, address _token) public view returns (uint256) {
        ...
        uint16 accountWeek = accountRewardWeek[_account][_token];
        uint8 tokenIdx = rewardToken[_token];
        if (accountWeek == 0) {
            accountWeek = rewardTokens[tokenIdx].startWeek;
        }
        for (uint16 j = accountWeek; j < currentWeek; j++) {
            TokenAmount memory reward = weeklyRewardData[j][tokenIdx];
            if (reward.amount == 0) {
                continue;
            }
            uint256 accountWeight = veLista.balanceOfAtWeek(_account, j);
    ...
    function claimWithToken(address _account, address token) public {
        uint256 tokenIdx = rewardToken[token];
        require(tokenIdx > 0, "token not registered");
        ...
        for (; accountWeek < currentWeek; ++accountWeek) {
            TokenAmount memory reward = weeklyRewardData[accountWeek][tokenIdx];
            if (reward.amount == 0) {
                continue;
            }
        ...
        accountRewardWeek[msg.sender][token] = accountWeek;
    ...
    function getTotalRewardByWeek(uint16 _week) external view returns (TokenAmount[] memory) {
        TokenAmount[10] memory rewardData = weeklyRewardData[_week];
        uint8 len;
        for (uint8 i = 1; i < rewardTokens.length; i++) {
